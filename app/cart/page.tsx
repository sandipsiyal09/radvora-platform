...
type Product={name:string;slug:string;sku:string;price_inr:number|null;currency:string;status:string;commerce_enabled:boolean;hsn_code:string|null;gst_rate:number|null;price_inr_includes_gst:boolean|null}
...
export default async function CartPage(){
  ...
  let items:CartItem[]=[]
  if(cart?.id){
    const {data}=await supabase.from('cart_items').select('id,quantity,unit_price,product_id,products(name,slug,sku,price_inr,currency,status,commerce_enabled,hsn_code,gst_rate,price_inr_includes_gst)').eq('cart_id',cart.id).order('created_at')
  ...
  const canCheckout=items.length>0 && items.every(item=>item.products?.status==='active'&&item.products?.commerce_enabled===true&&item.products?.currency==='INR'&&Number(item.products?.price_inr)>0&&Boolean(item.products?.hsn_code)&&item.products?.gst_rate!==null&&item.products?.price_inr_includes_gst!==null)
  ...
  return <main className="page-wrap"><div className="shell">
    <section className="page-head"><span className="kicker">RADVORA INDIA COMMERCE</span><h1>Your cart.</h1><p>RADVORA is currently available to consumers in India only. Pricing and applicable GST are resolved from the live governed catalog in INR; browser-supplied totals are ignored.</p></section>
    <div className="cart-layout">
      <section className="panel"><h2>Items</h2>{items.length?items.map((item,index)=>{const product=item.products;const estimate=estimates[index];return <div className="cart-row" key={item.id}><div><b>{product?.name||'Product'}</b><span>{product?.sku||item.product_id}</span>{product?.commerce_enabled!==true?<span>Currently unavailable for checkout</span>:null}{product?.gst_rate!==null&&product?.hsn_code?<span>HSN {product.hsn_code} · GST {Number(product.gst_rate)}% · price {product.price_inr_includes_gst?'includes':'excludes'} GST</span>:<span>India tax configuration pending</span>}<CartItemControls itemId={item.id} quantity={Number(item.quantity)}/></div><div><span>Qty {item.quantity}</span><b>{estimate.gross>0?money(estimate.gross):'Price pending'}</b></div></div>}):<p className="empty-state">Your cart is empty.</p>}</section>
      <aside className="panel cart-summary"><span className="kicker">INDIA ORDER SUMMARY</span><div className="summary-line"><span>Taxable value</span><b>{money(subtotal)}</b></div><div className="summary-line"><span>GST</span><b>{money(tax)}</b></div><div className="summary-line"><span>Estimated total</span><b>{money(total)}</b></div><div className="summary-line"><span>Currency</span><b>INR</b></div><div className="summary-line"><span>Delivery</span><b>India only</b></div><p className="empty-state">The server recalculates and snapshots HSN, GST rate, taxable value and tax for every line when the order is created. Shipping is currently ₹0 unless a future governed fulfilment rule explicitly changes it.</p>{items.length>0&&!canCheckout?<p className="status-message">Checkout remains blocked until every item is active, commerce-enabled, positively priced in INR and has complete approved GST/HSN configuration.</p>:null}<CheckoutButton disabled={!canCheckout} gatewayReady={gatewayReady}/><div className="actions"><Link className="pill ghost" href="/products/shieldtag-pro">Continue shopping</Link></div></aside>
    </div>
  </div></main>
}
