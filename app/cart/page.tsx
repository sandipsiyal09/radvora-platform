...
export default async function CartPage(){
  ...
  return <main className="page-wrap"><div className="shell">
    <section className="page-head"><span className="kicker">RADVORA INDIA COMMERCE</span><h1>Your cart.</h1><p>RADVORA is currently available to consumers in India only. All checkout amounts are resolved from the live catalog in INR; browser-supplied totals are ignored.</p></section>
    <div className="cart-layout">
      ...
      <aside className="panel cart-summary"><span className="kicker">INDIA ORDER SUMMARY</span><div className="summary-line"><span>Subtotal</span><b>{new Intl.NumberFormat('en-IN',{style:'currency',currency:'INR'}).format(subtotal)}</b></div><div className="summary-line"><span>GST</span><b>Calculated at checkout</b></div><div className="summary-line"><span>Shipping within India</span><b>Calculated at checkout</b></div><p className="empty-state">India-first checkout will support UPI and applicable domestic payment methods. Delivery is restricted to Indian addresses for the current launch phase.</p><CheckoutButton disabled={!canCheckout}/><div className="actions"><Link className="pill ghost" href="/products/shieldtag-pro">Continue shopping</Link></div></aside>
    </div>
  </div></main>
}
