...
export default async function CartPage(){
  ...
  let items:any[]=[]
  if(cart?.id){
    const {data}=await supabase.from('cart_items').select('id,quantity,unit_price,product_id,products(name,slug,sku,price_inr,currency,status,commerce_enabled)').eq('cart_id',cart.id).order('created_at')
    items=data||[]
  }
  const subtotal=items.reduce((sum,item)=>sum+(Number(item.products?.price_inr||0)*Number(item.quantity||0)),0)
  const canCheckout=items.length>0 && items.every(item=>item.products?.status==='active'&&item.products?.commerce_enabled===true&&item.products?.currency==='INR'&&Number(item.products?.price_inr)>0)
