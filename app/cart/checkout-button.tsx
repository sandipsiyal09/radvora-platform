...
export default function CheckoutButton({disabled=false,gatewayReady=false}:{disabled?:boolean;gatewayReady?:boolean}){
  ...
  const [form,setForm]=useState({name:'',phone:'',line1:'',line2:'',city:'',state:'',stateCode:'',postalCode:''})

  function setField(key:keyof typeof form,value:string){setForm(current=>({...current,[key]:value}))}
  function setStateCode(code:string){
    const state=INDIA_STATES.find(([candidate])=>candidate===code)?.[1]||''
    setForm(current=>({...current,stateCode:code,state}))
  }
  ...
  async function checkout(event:FormEvent){
    ...
    if(!form.stateCode||!form.state){setMessage('Select the Indian State or Union Territory for delivery.');return}
  ...
  return <form onSubmit={checkout} style={{display:'grid',gap:10,marginTop:16}} aria-label="India delivery and payment">
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
      <input className="field" required maxLength={100} aria-label="City" autoComplete="address-level2" placeholder="City" value={form.city} onChange={e=>setField('city',e.target.value)} disabled={!gatewayReady}/>
      <select className="field" required aria-label="State or Union Territory" autoComplete="address-level1" value={form.stateCode} onChange={e=>setStateCode(e.target.value)} disabled={!gatewayReady}>
        <option value="">State / Union Territory</option>
        {INDIA_STATES.map(([code,name])=><option key={code} value={code}>{name} ({code})</option>)}
      </select>
    </div>
