...
import './compatibility-cinematic.css'
...
const productPathByFamily:Record<string,string>={
  'ShieldTag Signature':'/products/shieldtag-signature',
  'ShieldTag Pro':'/products/shieldtag-pro',
  'ShieldTag Executive':'/products/shieldtag-executive',
  'ShieldTag Mini':'/products/shieldtag-mini'
}
...
export default function CompatibilityPage(){
  ...
  const reviewedProductPath=result?.device_family?productPathByFamily[result.device_family]:undefined
  const reviewedAvailabilityHref=useMemo(()=>{
    const path=reviewedProductPath||'/products'
    if(!result)return path
    const params=new URLSearchParams({manufacturer:result.manufacturer,model:result.device_model})
    if(result.region_variant)params.set('variant',result.region_variant)
    return `${path}?${params.toString()}`
  },[result,reviewedProductPath])
  ...
  return <PublicShell><main className={ui.main} data-cinematic-page="compatibility">
    ...
    <section className={ui.section}><div className={ui.grid2}>
      ...
      <aside className={ui.compatPanel} aria-live="polite">
        ...
        {!lookupError&&!variantRequired&&checked&&!result&&<><p>RADVORA has not published a reviewed compatibility record for this exact device yet. This is deliberately not treated as either compatible or incompatible.</p><div className={ui.actions}><Link className={ui.secondary} href="/#shieldlab">See styling only in ShieldLab →</Link></div><div className={ui.notice}>ShieldLab is a visual styling preview. It does not change this unreviewed compatibility status.</div></>}
        {result&&<><div className={ui.resultGrid}><div><span>STATUS</span><b>{result.compatibility_status}</b></div><div><span>DEVICE FAMILY</span><b>{result.device_family||category}</b></div><div><span>REGION / VARIANT</span><b>{result.region_variant||'General'}</b></div><div><span>INSTALLATION</span><b>{result.installation_note||'Use approved guidance'}</b></div></div>{result.compatibility_status!=='not_compatible'?<><div className={ui.actions}><Link className={ui.primary} href={reviewedAvailabilityHref}>View governed availability →</Link><Link className={ui.secondary} href="/#shieldlab">Style {suggestedFamily} in ShieldLab</Link><Link className={ui.secondary} href="/installation">Installation guide</Link></div><div className={ui.notice}>Compatibility is only the fit gate. Current India seller, tax, price and stock requirements are checked separately on the live product path before purchase.</div></>:<div className={ui.notice}>This reviewed record is marked not compatible. Do not rely on ShieldLab styling as fit guidance for this device.</div>}</>}
      </aside>
    </div></section>
  </main></PublicShell>
}