...
const devices:DeviceExample[]=[
  {id:'iphone',brand:'Apple',name:'iPhone 17',category:'Smartphone',image:'https://media.secure-mobiles.com/product-images/17A3.1.responsive-lg.centre.png',source:'https://www.apple.com/in/iphone-17/',tagClass:'tagIphone',fit:'contain',tone:'mist',syncFinish:{id:'mist-sync',name:'Mist Blue Sync',base:'#a9bdd6',edge:'#e0edf8',accent:'#d9f4ff',text:'#13202b'}},
  {id:'galaxy',brand:'Samsung',name:'Galaxy S26',category:'Smartphone',image:'https://store005.ru/image/cache/catalog/products_images/samsung/smartfon-samsung-galaxy-s26-sm-s942b-12-512gb-silver-shadow-42345_3-1000x1000.jpg',source:'https://www.samsung.com/in/smartphones/galaxy-s26/',tagClass:'tagGalaxy',fit:'contain',tone:'silver',syncFinish:{id:'silver-sync',name:'Silver Shadow Sync',base:'#aeb2b9',edge:'#eef1f4',accent:'#d9f7ff',text:'#11161c'}},
  {id:'oppo',brand:'OPPO',name:'Reno16 5G',category:'Smartphone',image:'https://darlingretail.com/cdn/shop/files/25_ab6ff382-f87c-45f0-b75f-2e71a27ea490_800x.jpg?v=1783056588',source:'https://www.oppo.com/in/smartphones/series-reno/reno16/specs/',tagClass:'tagOppo',fit:'contain',tone:'purple',syncFinish:{id:'violet-sync',name:'Twilight Violet Sync',base:'#4d354f',edge:'#a781aa',accent:'#d9aee2',text:'#fff9ff'}},
  {id:'vivo',brand:'vivo',name:'V50',category:'Smartphone',image:'https://rimage.ripley.com.pe/home.ripley/Attachment/MKP/5258/PMP20001132026/imagen3-1.jpeg',source:'https://www.vivo.com/en/products/picture/v50',tagClass:'tagVivo',fit:'contain',tone:'blue',syncFinish:{id:'blue-sync',name:'Starry Blue Sync',base:'#184c9d',edge:'#6aa3e8',accent:'#8fd5ff',text:'#f5fbff'}},
  {id:'ipad',brand:'Apple',name:'iPad Air',category:'Tablet',image:'https://backend.paiinternational.in/media/images/3_ryPmXCT.jpg',source:'https://www.apple.com/in/ipad-air/',tagClass:'tagIpad',fit:'contain',tone:'blue',syncFinish:{id:'ipad-blue-sync',name:'Blue Alloy Sync',base:'#79a7bd',edge:'#cae4ef',accent:'#bcefff',text:'#10202a'}},
  {id:'macbook',brand:'Apple',name:'MacBook Air',category:'Laptop',image:'https://yabloki.ua/media/cache/sylius_shop_product_original/c2/b9/macbook-air-13-apple-m4-10cpu-10gpu-24gb-512gb-ssd-silver-mc654_b9ae99ee-ea9d-42f3-a119-150804a600de-6.png.webp',source:'https://www.apple.com/in/macbook-air/specs/',tagClass:'tagMac',fit:'contain',tone:'silver',syncFinish:{id:'mac-silver-sync',name:'Silver Alloy Sync',base:'#b8bdc2',edge:'#f0f3f5',accent:'#d9f6ff',text:'#141a1f'}},
  {id:'airpods',brand:'Apple',name:'AirPods Pro case',category:'Accessory',image:'https://cdn8.web4s.vn/media/products/2436/muyg3_av4.jpg',source:'https://www.apple.com/in/airpods-pro/specs/',tagClass:'tagPods',fit:'contain',tone:'white',syncFinish:{id:'pearl-sync',name:'Pearl White Sync',base:'#e9ebed',edge:'#ffffff',accent:'#c9f5ff',text:'#162028'}},
  {id:'powerbank',brand:'Samsung',name:'20,000mAh Battery Pack',category:'Accessory',image:'https://images-na.ssl-images-amazon.com/images/I/7115Yo0tuvL.jpg',source:'https://www.samsung.com/in/mobile-accessories/battery-pack-20-000mah-beige-eb-p4520xuegin/',tagClass:'tagPower',fit:'contain',tone:'beige',syncFinish:{id:'beige-sync',name:'Warm Beige Sync',base:'#d3cbbd',edge:'#f6f1e7',accent:'#e8fbff',text:'#28231d'}}
]
...
function Ecosystem({finish}:{finish:Finish}){
  return <section className={ui.ecosystem} id="devices">
    ...
    <div className={ui.ecoGrid}>{devices.slice(0,6).map(d=><article key={d.id} className={ui.ecoCard}><div className={ui.ecoVisual}><DeviceVisual device={d} finish={finish} label={d.category==='Laptop'?'EXECUTIVE':d.category==='Tablet'?'PRO':'SIGNATURE'}/></div><div className={ui.ecoInfo}><div><span>{d.category}</span><h3>{d.brand}</h3><p>{d.name}</p></div><a href={d.source} target="_blank" rel="noreferrer">Device reference ↗</a></div></article>)}</div>
  </section>
}
...
function FinishStudio({finishId,setFinishId}:{finishId:string;setFinishId:(id:string)=>void}){
  ...
  return <section className={ui.finishSection} id="finishes"><div className={ui.finishCopy}><span>MATCH YOUR DEVICE</span><h2>Blend in.<br/><em>Stand out.</em></h2><p>Preview the same ShieldTag geometry in eight finish directions. These remain design previews until the sellable catalogue is formally approved.</p><div className={ui.finishMeta}><span>Selected finish</span><b>{finish.name}</b><p>Lighting, tag face and edge treatment update together so the finish reads as a material—not a flat colour chip.</p></div></div><div className={ui.finishStage}><div className={ui.finishDevice}><DeviceVisual device={iphone} finish={finish} label="SIGNATURE" interactive useDeviceFinish={false}/></div><div className={ui.finishLight}/><div className={ui.finishSwatches}>{finishes.map(f=><button key={f.id} className={f.id===finishId?ui.swatchActive:''} onClick={()=>setFinishId(f.id)} aria-pressed={f.id===finishId}><i style={{'--swatch':f.base,'--edge':f.edge} as CSSProperties}/><span>{f.name}</span></button>)}</div></div></section>
}
