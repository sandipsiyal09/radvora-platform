...
type DeviceExample={id:string;brand:string;name:string;category:string;deviceColor:string;image:string;source:string;tagClass:string;fit:'contain'|'cover';tone:string;syncFinish:Finish}
...
function DeviceMatcher({finishId,setFinishId}:{finishId:string;setFinishId:(id:string)=>void}){
  ...
  useEffect(()=>{if(!availableDevices.some(d=>d.id===deviceId))setDeviceId(availableDevices[0]?.id??devices[0].id)},[availableDevices,deviceId])
  const match=useMemo(()=>devices.find(d=>d.id===deviceId)??availableDevices[0]??devices[0],[deviceId,availableDevices])

  return <section className={ui.matcherSection} id="matcher">
    <div className={ui.matcherPanel}><div className={ui.matcherIntro}><span>FIND YOUR MATCH</span><h2>Build the visual match.</h2><p>Choose the device class, brand example, shown product, device colour and ShieldTag finish. Appearance can be previewed here; exact compatibility remains evidence-gated.</p></div>
      <div className={ui.matcherControls}>
        <label><span>01 Device category</span><select value={category} onChange={e=>setCategory(e.target.value)}><option>Smartphone</option><option>Tablet</option><option>Laptop</option><option>Accessory</option></select></label>
        <label><span>02 Brand example</span><select value={brand} onChange={e=>setBrand(e.target.value)}>{availableBrands.map(b=><option key={b}>{b}</option>)}</select></label>
