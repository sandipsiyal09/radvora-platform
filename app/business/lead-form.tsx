...
export default function BusinessLeadForm({source='business-page',title='Corporate, retail, distributor & OEM'}:{source?:string;title?:string}){
  ...
  return <form className="panel" onSubmit={submit} style={{display:'grid',gap:12}} aria-label={isPrivacy?'Privacy request':'Business enquiry'}>
    ...
    <label className="field-label" htmlFor={`${idPrefix}-name`}>Full name</label>
    <input id={`${idPrefix}-name`} className="field" name="name" required minLength={2} maxLength={120} autoComplete="name" placeholder="Full name"/>
    {!isPrivacy?<><label className="field-label" htmlFor={`${idPrefix}-company`}>Company</label><input id={`${idPrefix}-company`} className="field" name="company" maxLength={160} autoComplete="organization" placeholder="Company"/></>:null}
    <label className="field-label" htmlFor={`${idPrefix}-email`}>{isPrivacy?'Email address':'Work email'}</label>
    <input id={`${idPrefix}-email`} className="field" name="email" type="email" required maxLength={320} autoComplete="email" placeholder={isPrivacy?'Email address':'Work email'}/>
    <label className="field-label" htmlFor={`${idPrefix}-phone`}>Phone (optional)</label>
