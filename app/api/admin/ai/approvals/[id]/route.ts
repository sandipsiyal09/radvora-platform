...
function canonicalOrigin(request:Request){
  ...
  if(configured){
    try{
      ...
      // Privileged production AI approval decisions must fail closed instead of trusting request.url.
    }
  }
...
function boundary(request:Request,canonical:string){
  ...
  if(rawLength){
    ...
    if(!Number.isSafeInteger(declaredLength)||declaredLength<0)return json({error:'Invalid approval request length.'},400)
  ...
  if(invalidOrigin(request,canonical))return json({error:'Invalid approval request origin.'},403)
  return null
}

export async function POST(request:Request,{params}:Params){
  const expectedOrigin=canonicalOrigin(request)
  if(!expectedOrigin)return json({error:'Approval administration is temporarily unavailable.'},503)
  ...
  if(error){console.error('server_approval_decision_failed');return json({error:'Unable to resolve this approval request.'},409)}
