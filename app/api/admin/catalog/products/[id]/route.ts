...
type Body={action?:'catalog'|'tax'|'commerce';name?:string;shortDescription?:string;description?:string;priceInr?:number|null;status?:string;hsnCode?:string;gstRate?:number;priceIncludesGst?:boolean;enabled?:boolean}
...
export async function POST(request:Request,{params}:Params){
  ...
  if(body.action==='catalog'){
    ...
    const priceInr=body.priceInr===null?null:typeof body.priceInr==='number'?body.priceInr:NaN
  ...
  if(body.action==='tax'){
    const hsnCode=typeof body.hsnCode==='string'?body.hsnCode.trim():''
    const gstRate=typeof body.gstRate==='number'?body.gstRate:NaN
  ...
  if(body.action==='commerce'&&typeof body.enabled==='boolean'){
    ...
    if(error){console.error('server_product_commerce_toggle_failed',error);return json({error:body.enabled?'Product is not ready for India commerce. Confirm active status, positive INR pricing, HSN and GST configuration.':'Unable to disable India commerce.'},409)}
