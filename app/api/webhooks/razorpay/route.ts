...
type RazorpayEvent={event?:string;payload?:{payment?:{entity?:RazorpayPayment};refund?:{entity?:RazorpayRefund}}}
...
async function processCapturedPayment(admin:ReturnType<typeof createAdminClient>,eventId:string,payment:RazorpayPayment){
  ...
  try{
    ...
    const {error:finalizeError}=await admin.rpc('finalize_razorpay_payment',{p_order_id:attempt.order_id,p_attempt_id:attempt.id,p_payment_id:paymentId})
    if(finalizeError) throw finalizeError

    await markEvent(admin,eventId,'processed')
    return json({ok:true})
  }catch(error){
    console.error('razorpay_payment_webhook_processing_failed',error)
    await markEvent(admin,eventId,'failed','Payment reconciliation failed')
    return json({error:'Webhook processing failed.'},500)
  }
}

async function processRefund(admin:ReturnType<typeof createAdminClient>,eventId:string,eventType:string,refund:RazorpayRefund){
  const refundId=refund.id||''
  const paymentId=refund.payment_id||''
  const providerStatus=String(refund.status||'')
  const amountPaise=refund.amount
  if(!refundId||!paymentId||!Number.isSafeInteger(amountPaise)||Number(amountPaise)<=0||String(refund.currency).toUpperCase()!=='INR'){
    await markEvent(admin,eventId,'failed','Invalid refund payload')
    return json({error:'Invalid refund payload.'},400)
  }
  if(eventType==='refund.processed'&&providerStatus!=='processed'){
    await markEvent(admin,eventId,'failed','Refund status mismatch')
    return json({error:'Invalid refund status.'},400)
  }
  if(eventType==='refund.failed'&&providerStatus!=='failed'){
    await markEvent(admin,eventId,'failed','Refund status mismatch')
    return json({error:'Invalid refund status.'},400)
  }
  ...
  try{
    ...
    const {data:byProvider,error:providerLookupError}=await admin.from('refund_attempts')
      .select('id,payment_attempt_id,order_id,amount,currency,status').eq('provider','razorpay').eq('provider_refund_id',refundId).maybeSingle()
    if(providerLookupError) throw providerLookupError
    refundAttempt=byProvider

    if(!refundAttempt){
      const noteAttemptId=typeof refund.notes?.refund_attempt_id==='string'?refund.notes.refund_attempt_id:''
      if(noteAttemptId){
        const {data:byNote,error:noteLookupError}=await admin.from('refund_attempts')
          .select('id,payment_attempt_id,order_id,amount,currency,status').eq('id',noteAttemptId).eq('provider','razorpay').maybeSingle()
        if(noteLookupError) throw noteLookupError
        refundAttempt=byNote
      }
    }
    ...
    const {data:payment,error:paymentError}=await admin.from('payment_attempts')
      .select('id,provider_payment_id,amount,currency,status').eq('id',refundAttempt.payment_attempt_id).eq('provider','razorpay').maybeSingle()
    if(paymentError||!payment) throw new Error('Refund payment attempt not found')
    ...
    if(refundAttempt.status==='processed'&&eventType==='refund.processed'){
      await markEvent(admin,eventId,'processed')
      return json({ok:true,alreadyProcessed:true})
    }
    ...
    const {error:finalizeError}=await admin.rpc('finalize_razorpay_refund',{
      ...
      p_failure_code:targetStatus==='failed'?'provider_refund_failed':null
    })
    if(finalizeError) throw finalizeError
    ...
    console.error('razorpay_refund_webhook_processing_failed',error)
    await markEvent(admin,eventId,'failed','Refund reconciliation failed')
    return json({error:'Webhook processing failed.'},500)
  }
}
...
export async function POST(request:Request){
  ...
  if(eventType==='payment.captured'){
    ...
    return processCapturedPayment(admin,eventId,payment)
  }
