...
function canonicalOrigin(request:Request){
  ...
  if(configured){
    try{
      ...
      // Production cancellation mutations fail closed instead of trusting request.url.
    }
  }
...
export async function POST(request:Request,{params}:Params){
  ...
  if(attempt?.provider_session_id){
    ...
    try{
      const fetchResponse=await fetch(`https://api.razorpay.com/v1/payment_links/${encodeURIComponent(attempt.provider_session_id)}`,{headers:{Authorization:`Basic ${auth}`},cache:'no-store',signal:providerAbort.signal})
      ...
      if(link.status==='created'){
        const cancelResponse=await fetch(`https://api.razorpay.com/v1/payment_links/${encodeURIComponent(attempt.provider_session_id)}/cancel`,{method:'POST',headers:{Authorization:`Basic ${auth}`,'Content-Type':'application/json'},cache:'no-store',signal:providerAbort.signal})
      ...
      if(closeError){console.error('payment_link_cancel_reconcile_failed');return json({error:'The provider session is closed, but local cancellation reconciliation is pending. Do not retry payment; contact Support.'},502)}
      return json({ok:true,orderId:closed?.id||orderId,status:'cancelled'})
    }catch{
      console.error('payment_link_cancel_failed')
      return json({error:'Unable to verify or cancel the provider payment session. Inventory remains reserved for safety.'},502)
    }finally{
  ...
  if(error){
    ...
    console.error('pending_order_cancel_failed');return json({error:'Unable to cancel the order safely.'},500)
  }
