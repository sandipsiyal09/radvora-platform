...
type TaxItem={line_subtotal:number|string|null;tax_amount:number|string|null;line_total:number|string;hsn_code:string|null;gst_rate:number|string|null;price_includes_gst:boolean|null}
...
function checkoutPayload(keyId:string,order:CheckoutOrder,providerOrderId:string,amountPaise:number,email:string,name:string,phone:string,reused=false){return {keyId,providerOrderId,amountPaise,currency:'INR',orderId:order.id,orderNumber:order.order_number,customer:{name,email,phone},reused}}
...
export async function POST(request:Request){
  ...
  let rawBody:string;try{rawBody=await request.text()}catch{return json({error:'Invalid checkout request.'},400)}
  ...
  if(body.orderId){
    ...
    if(orderError){console.error('checkout_active_cart_india failed',orderError);return json({error:'Unable to create the India order from the current cart.'},400)}
  ...
  const [{data:storedOrder,error:storedOrderError},{data:itemRows,error:itemError}]=await Promise.all([
    ...
    admin.from('order_items').select('line_subtotal,tax_amount,line_total,hsn_code,gst_rate,price_includes_gst').eq('order_id',order.id)
  ])
  ...
  const taxItems=(itemRows||[]) as TaxItem[]
  if(!taxItems.length||taxItems.some(item=>item.line_subtotal===null||item.tax_amount===null||!item.hsn_code||item.gst_rate===null||item.price_includes_gst===null)) return json({error:'This order is missing the required India GST/HSN snapshot and cannot be paid.'},409)
  const lineSubtotal=Math.round(taxItems.reduce((sum,item)=>sum+Number(item.line_subtotal||0),0)*100)/100
  const lineTax=Math.round(taxItems.reduce((sum,item)=>sum+Number(item.tax_amount||0),0)*100)/100
  const lineTotal=Math.round(taxItems.reduce((sum,item)=>sum+Number(item.line_total||0),0)*100)/100
  ...
  if(!moneyEqual(lineSubtotal,storedSubtotal)||!moneyEqual(lineTax,storedTax)||!moneyEqual(lineTotal+storedShipping,storedTotal)||!moneyEqual(storedSubtotal+storedTax+storedShipping,storedTotal)){
    console.error('india_order_accounting_mismatch',{orderId:order.id,lineSubtotal,lineTax,lineTotal,storedSubtotal,storedTax,storedShipping,storedTotal})
    return json({error:'Order accounting requires review before payment can continue.'},409)
  }
  ...
  const {data:existing,error:existingError}=await admin.from('payment_attempts').select('id,provider_order_id,status,updated_at').eq('order_id',order.id).in('status',['created','pending']).order('created_at',{ascending:false}).limit(1).maybeSingle<ActiveAttempt>()
  if(existingError){console.error('razorpay_active_attempt_lookup_failed',existingError);return json({error:'Unable to verify payment state safely.'},500)}
  ...
  if(existing){
    if(existing.provider_order_id){
      try{
        ...
        if(response.ok&&providerOrder.id===existing.provider_order_id&&providerOrder.amount===amountPaise&&String(providerOrder.currency).toUpperCase()==='INR'&&['created','attempted'].includes(String(providerOrder.status))) return json(checkoutPayload(keyId,order,existing.provider_order_id,amountPaise,user.email||'',name,phone,true))
      ...
      }catch(error){console.error('razorpay_existing_order_lookup_failed',error);return json({error:'Unable to verify the existing payment session. Please try again shortly.',orderId:order.id,retryable:true},502)}
    }else{
  ...
  if(attemptError||!attempt){if(attemptError?.code==='23505')return json({error:'A payment attempt is already active for this order.',orderId:order.id,retryable:true},409);console.error('razorpay_attempt_create_failed',attemptError);return json({error:'Unable to initialize payment tracking.'},500)}

  try{
    const providerResponse=await fetch('https://api.razorpay.com/v1/orders',{method:'POST',headers:{Authorization:`Basic ${auth}`,'Content-Type':'application/json'},body:JSON.stringify({amount:amountPaise,currency:'INR',receipt:String(order.order_number).slice(0,40),notes:{order_id:order.id,payment_attempt_id:attempt.id}}),cache:'no-store'})
