...
const EXPECTED_RUNTIME_SCHEMA_VERSION='202609110049'
...
export default async function ReadinessPage(){
  ...
  const [{data:products},{count:pendingApprovals},{count:failedWebhooks},{count:openRefunds},{count:failedRefunds},{data:agents},{data:guardrails},{data:reservationRows},{data:sellerReady,error:sellerReadyError},{data:runtimeSchemaVersion,error:runtimeSchemaError}]=await Promise.all([
  ...
  const sellerProfileReady=!sellerReadyError&&sellerReady===true
  const sellable=catalog.filter(p=>sellerProfileReady&&p.status==='active'&&p.commerce_enabled===true&&p.currency==='INR'&&Number(p.price_inr)>0&&hasTaxConfig(p)&&hasInventory(p))
  ...
  return <main className="page-wrap"><div className="shell">
    <section className="page-head"><span className="kicker">RADVORA INDIA LAUNCH</span><h1>Launch readiness.</h1><p>This view reports runtime schema, seller identity, configuration, statutory catalog, inventory, payment-operation and AI-safety blockers without exposing credentials. It does not bypass scientific, compliance, payment, tax, stock or human approval controls.</p></section>
