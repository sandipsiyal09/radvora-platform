...
type PaymentState={order_id:string;provider:string;status:string;created_at:string}
...
export default async function AdminOperationsPage(){
  ...
  if(staleOrderIds.length){
    const {data}=await supabase.from('payment_attempts').select('order_id,provider,status,created_at').in('order_id',staleOrderIds).order('created_at',{ascending:false})
