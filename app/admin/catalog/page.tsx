...
export default async function CatalogAdminPage(){
  ...
  const {data:products}=await supabase.from('products').select('id,name,slug,sku,short_description,description,status,price_inr,currency,updated_at').order('updated_at',{ascending:false})
  return <main className="page-wrap"><div className="shell">
    <section className="page-head"><span className="kicker">RADVORA CATALOG · INTERNAL</span><h1>Product control.</h1><p>Pricing, descriptions and publication state are controlled here. Scientific or health claims do not belong in product copy unless separately approved through Labs.</p></section>
    <div className="admin-grid">{products?.length?products.map(p=><section className="panel" key={p.id}><div className="admin-row"><div><b>{p.name}</b><span>{p.sku} · /{p.slug}</span></div><em>{p.status}</em></div><ProductEditor product={{...p,price_inr:p.price_inr===null?null:Number(p.price_inr)}}/></section>):<section className="panel"><p className="empty-state">No products found.</p></section>}</div>
  </div></main>
}
