...
const sections = [
  ...
  ['Unpaid order cancellation', 'A customer may cancel a pending unpaid order only while no active payment-provider session or completed payment state exists for that order. Once a secure payment session is active, payment status must be reconciled safely before cancellation can become eligible. If payment is captured or fulfilment progresses, cancellation or return eligibility is handled under the applicable refund, return, and consumer-rights process instead.'],
