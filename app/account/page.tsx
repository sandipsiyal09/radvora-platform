...
export default async function AccountPage() {
  ...
  return <main className="page-wrap"><div className="shell">
    ...
    <div className="account-grid">
      <section className="panel"><span className="kicker">REGISTERED PRODUCTS</span><h2>{registrations?.length ?? 0}</h2>{registrations?.length ? registrations.map(r=><div className="account-row" key={r.id}><div><b>Registered RADVORA product</b><span>{new Date(r.registered_at).toLocaleDateString('en-IN')}</span></div><em>active</em></div>) : <p className="empty-state">No products registered yet.</p>}</section>
