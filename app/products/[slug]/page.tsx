...
export async function generateMetadata({params}:PageProps):Promise<Metadata>{
  ...
  return {title:product.name,description,alternates:{canonical:`/products/${product.slug}`},openGraph:{title:`${product.name} | RADVORA Technologies`,description,url:`/products/${product.slug}`,type:'website'}}
}

export default async function ProductDetailPage({ params }: PageProps){
  const { slug } = await params
  const supabase = await createClient()
  const { data: product } = await supabase.from('products').select('id,name,slug,sku,short_description,description,status,price_inr,currency,commerce_enabled,stock_on_hand,stock_reserved').eq('slug',slug).eq('status','active').maybeSingle()
  if(!product) notFound()
