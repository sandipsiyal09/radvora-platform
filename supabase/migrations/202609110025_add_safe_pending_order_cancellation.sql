...
begin
  ...
  if exists(select 1 from public.payment_attempts where order_id=p_order_id and status in ('authorized','captured','refunded')) then
    raise exception 'Order has a completed payment state';
  end if;
  ...
  perform private.write_audit('order_cancelled_by_customer','order',p_order_id::text,'Customer cancelled pending unpaid order',jsonb_build_object('user_id',p_user_id));
  return v_order;
end;$$;
