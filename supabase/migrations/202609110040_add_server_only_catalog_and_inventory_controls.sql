create or replace function public.server_update_product_catalog(
  p_actor_id uuid,p_actor_role text,p_product_id uuid,p_name text,p_short_description text,p_description text,p_price_inr numeric,p_status text
)
returns public.products language plpgsql security definer set search_path=public,private,pg_temp as $$
declare v_product public.products%rowtype;v_name text:=trim(coalesce(p_name,''));
begin
  ...
  update public.products set name=v_name,short_description=nullif(trim(coalesce(p_short_description,'')),''),description=nullif(trim(coalesce(p_description,'')),''),price_inr=p_price_inr,status=p_status,commerce_enabled=false,updated_at=now() where id=p_product_id returning * into v_product;
  if not found then raise exception 'Product not found'; end if;
  perform private.write_audit_actor(p_actor_id,'product_catalog_update','product',p_product_id::text,'Updated product catalog record; commerce disabled pending review',jsonb_build_object('status',p_status,'price_inr',p_price_inr,'commerce_enabled',false));
  return v_product;
end;$$;
...
create or replace function public.server_set_product_india_tax_config(
  p_actor_id uuid,p_actor_role text,p_product_id uuid,p_hsn_code text,p_gst_rate numeric,p_price_includes_gst boolean
)
returns public.products language plpgsql security definer set search_path=public,private,pg_temp as $$
declare v_product public.products%rowtype;v_hsn text:=nullif(trim(coalesce(p_hsn_code,'')),'');
begin
  if p_actor_id is null or p_actor_role not in ('admin','founder') then raise exception 'Admin access required'; end if;
  ...
  update public.products set hsn_code=v_hsn,gst_rate=p_gst_rate,price_inr_includes_gst=p_price_includes_gst,commerce_enabled=false,updated_at=now() where id=p_product_id returning * into v_product;
  if not found then raise exception 'Product not found'; end if;
  perform private.write_audit_actor(p_actor_id,'product_tax_config','product',p_product_id::text,'Updated India GST/HSN configuration; commerce disabled pending review',jsonb_build_object('hsn_code',v_hsn,'gst_rate',p_gst_rate,'price_includes_gst',p_price_includes_gst));
  return v_product;
end;$$;
...
create or replace function public.server_set_product_inventory(
  p_actor_id uuid,p_actor_role text,p_product_id uuid,p_stock_on_hand integer
)
returns public.products language plpgsql security definer set search_path=public,private,pg_temp as $$
...
begin
  ...
  perform private.write_audit_actor(p_actor_id,'product_inventory_update','product',p_product_id::text,'Updated governed stock; commerce disabled pending review',jsonb_build_object('stock_on_hand',p_stock_on_hand,'stock_reserved',v_product.stock_reserved));
  return v_product;
end;$$;
revoke all on function public.server_set_product_inventory(uuid,text,uuid,integer) from public,anon,authenticated;
grant execute on function public.server_set_product_inventory(uuid,text,uuid,integer) to service_role;
